...
#######
#Subsetting for one day 


#Subset the 12th day of January 2008 - 10 Minute frequency
houseDay10 <- filter(Yrs2007_2009, Year == 2008 & Month == 1 & Day == 14 & (Minute == 0 | 
                                                                          Minute == 10 | 
                                                                          Minute == 20 | 
                                                                          Minute == 30 | 
                                                                          Minute == 40 | 
...
##Only Submeter 1 per day
plot_ly(houseDay10, x = ~houseDay10$DateTime, y = ~houseDay10$Sub_metering_1, name = 'Kitchen', type = 'scatter', mode = 'lines') %>%
  layout(title = "Power Consumption Kitchen January 14th, 2008",
         xaxis = list(title = "Time"),
         yaxis = list (title = "Power (watt-hours)"))

##Only Submeter 2 per day
plot_ly(houseDay10, x = ~houseDay10$DateTime, y = ~houseDay10$Sub_metering_2, name = 'Laundry Room', type = 'scatter', mode = 'lines') %>%
  layout(title = "Power Consumption Laundry Room January 14th, 2008",
         xaxis = list(title = "Time"),
         yaxis = list (title = "Power (watt-hours)"))

##Only Submeter 3 per day
plot_ly(houseDay10, x = ~houseDay10$DateTime, y = ~houseDay10$Sub_metering_3, name = 'Water Heater & AC', type = 'scatter', mode = 'lines') %>%
...
##Creating subset and plotting consumption for Year 2007
data_2007 <- Yrs2007_2009 %>%
  filter(Year == 2007)

plot_ly(data_2007, x = ~data_2007$DateTime, y = ~data_2007$Sub_metering_1, name = 'Kitchen', type = 'scatter', mode = 'lines') %>%
  add_trace(y = ~data_2007$Sub_metering_2, name = 'Laundry Room', mode = 'lines') %>%
  add_trace(y = ~data_2007$Sub_metering_3, name = 'Water Heater & AC', mode = 'lines') %>%
  layout(title = "Power Consumption 2007 per month",
         xaxis = list(title = "Time"),
         yaxis = list (title = "Power (watt-hours)"))
